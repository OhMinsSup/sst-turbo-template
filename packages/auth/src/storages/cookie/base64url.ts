...
const FROM_BASE64URL = (() => {
  const charMap: number[] = new Array(128) as unknown as number[];
  ...
  for (const char of TO_BASE64URL) {
    charMap[char.charCodeAt(0)] = TO_BASE64URL.indexOf(char);
  }
...
export function stringFromBase64URL(str: string) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    const bits = FROM_BASE64URL[codepoint] as unknown as number;
